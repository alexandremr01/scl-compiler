// Based on code by Guilherme Nascimento de Oliveira
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 3) {
        printf("Usage: asm src_file output_file\n");
        return 1;
    }
    if (!(file_r = fopen(argv[1],"r"))){
        printf("FATAL: Could not open %s \n", argv[1]);
        return 1;
    }
    if (!(file_w = fopen(argv[2],"w"))) {
        printf("FATAL: Could not open %s \n", argv[1]);
        return 1;
    }
    ...
    while (!feof(file_r)) {
        ...
        fwrite(&bytecode, sizeof(bytecode), 1, file_w);
    }
